...
let occurences = 0;
const passwordSplit = passwords.split('\n').filter(x => x.length > 0);

passwordSplit.forEach((passwordString) => {

    const [passwordRule, password] = passwordString.trim().split(':');
    const [numbers, characters] = passwordRule.split(' ');
    const [from, to] = numbers.split('-').map(amount => +amount);
    const numberOfCharacters = password.split('').filter(x => x === characters).length;
    const isValidPassword = numberOfCharacters >= from && numberOfCharacters <= to;
